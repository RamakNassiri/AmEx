...
data_file = "best_200_dataset.pkl"
...
for epoch in range(epochs):
    ...
    for i, (X, y) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        outputs = model(X, return_all_blocks=True)  # Requesting all blocks' outputs
        output = outputs[3].mean(dim=1)  # Using output from the 4th block
    ...
    with torch.no_grad():
        for X_val, y_val in test_loader:
            ...
            outputs = model(X_val, return_all_blocks=True)  # Requesting all blocks' outputs
            output = outputs[2].mean(dim=1)  # Using output from the 4th block
    ...
    if eval_f1 > best_eval_f1:
        ...
        torch.save(model.state_dict(), 'best_model_third_block.pth')

    print(f"Epoch {epoch} - Eval loss: {eval_loss} - Eval F1: {eval_f1}")

print("Training for the third block complete")
